...
label_factor = [1, 80, 5]  # 对label放缩的基础系数
...
#                                                                              normalizedParams)
input_data_array = spectrum_data_normalized.reshape(
    (spectrum_data_normalized.shape[0], spectrum_data_normalized.shape[1], 1))
output_data_array = np.array(label_data)[:, 0:peakNum]
...
# 定义第一部分卷积全连接网络
dense_layer_1_input = Conv1D(filters=8, kernel_size=10, activation='relu', )(input_tensor)
...
# 定义第二部分神经网络，接收第一个网络的信息，它用于输出峰中心位置
out_shape_2_4 = output_data_array.shape[1]  # 输出维度
...
model_checkpoint = tf.keras.callbacks.ModelCheckpoint(
    filepath='Model_contrast/' + peakNum_director_name + 'model_fitParams_peakPosition.keras',  # 保存的模型文件路径
...
plt.ylabel('Loss')
plt.legend()
plt.savefig('Model_contrast/' + peakNum_director_name + 'Training_and_Validation_Loss_PeakPosition.png')
# -------------------------------------------------------------------

# load模型
model = tf.keras.models.load_model('Model_contrast/' + peakNum_director_name + 'model_fitParams_peakPosition.keras')
...
test_input_data_array = test_spectrum_data_normalized
test_output_data_array = test_label_data[:, 0:peakNum]

# 使用模型对测试数据进行预测
model_predict_data = model.predict(test_input_data_array)
model_predict_params = model_predict_data
...
for index in sample_indices:
    ...
    plt.axvline(model_predict_params[index][0], label='predicted resonance frequency')
    for i in range(1,peakNum):
        plt.axvline(model_predict_params[index][i])
    print('true params:', test_output_data_array[index])
    print('predict params:', model_predict_params[index])

    plt.xlabel('frequency(GHz)')
    plt.ylabel('Intensity')
    plt.title(f'predict the resonance frequency of input spectrum({index + 1})')
    plt.legend()
    plt.savefig('Model_contrast/' + peakNum_director_name + f'figure_sample{index + 1}_peakPosition.png')
    print('\n')
plt.show()
# ------------结束-------------------------------------------------------------------------------------
'''# ------注：本文件没有用下面的代码--------------------拟合谱线---------------------------------------------------------------------
